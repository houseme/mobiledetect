...
func TestVersionExtraction(t *testing.T) {
	...
	for _, data := range VersionData() {
		...
		if strVersion != detectedVersion {
			t.Errorf("ua version %s is mismatched (detectedVersion %s, property %s)", strVersion, detectedVersion, property)
		}

		detectedVersionFloat := detect.VersionFloat(property)
		if floatVersion != detectedVersionFloat {
			t.Errorf("Float version %v is mismatched (detectedVersion %v, property %s)", floatVersion,
				detectedVersionFloat, property)
		}
	}
}
